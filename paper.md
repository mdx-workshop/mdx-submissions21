...
Figure \ref{fig:architecture} describes the flow of the proposed model.

![architecture](figs/Figure2.png){width=40%}
...
We also conduct an experiment using methods without the STFT and complement loss, introduced in Section \ref{sec:propsed methods} to compare the effects of them.
To this end, we first separate sources from mixtures in the  MUSDB18 test dataset using each mdoel.
