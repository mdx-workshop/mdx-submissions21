...
The backbones of the proposed framework, dubbed Danna-Sep^[<https://github.com/yoyololicon/danna-sep>], are two spectrogram-based models, i.e., a modified X-UMX and U-Net, and an enhanced Demucs as the waveform-based model.
Given an input of mixture, we linearly combined outputs from the three individual models to obtain the final result.
...
Nevertheless, the framework is rather computationally demanding due to the combination, and we leave it as our future work to improve the efficiency.
...
## Demucs
For Demucs [@defossez2019music], we built upon the variant with 48 hidden channels, and enhanced the model by replacing the decoder with four independent decoders responsible for four individual sources.
...
The training loss is a L1-norm between predicted waveforms and source-target waveforms, and it took approximately 10 days to train on a single RTX 3070 using mixed precision with a batch size of 16, and four steps of gradient accumulation.
...
Experiments were conducted to search for optimal weighting. The optimal weights for each source, type of input domain (T for waveforms, TF for frequency masking), and the size of the models are given in the following table.
...
We performed the benchmark using musdb18 [@musdb18] as the dataset, and evaluated our models in terms of Signal-to-Distortion Ratio (SDR) [@vincent2006performance] derived from *museval* [@fabian_robert_stoter_2019_3376621].
One iteration of MWF was used for X-UMX and U-Net, and we didn't apply the shift trick [@defossez2019music] for our Demucs model. The results of our models and the original X-UMX and Demucs are shown in the table below.
...
Our enhanced Demucs outperformed the original model by 0.03 dB in terms of avereage SDR, despite the fact that the shift trick was not applied.
Notably, Danna-Sep surpassed both the original and enhanced Demucs by a large margin (+0.53 dB on average SDR).
These results show the effectiveness of our modifications for the training scheme and architecture, as well as the simple fusion method.
The proposed framework, however, trades computation for the significant improvement in terms of SDR, and we leave it for future work to mitigate the issue.
