...
# Introduction
Music source separation aims at separating a mixture track into several soundtracks, such as `Vocals`, `Bass`, `Drums`, and `Other` tracks. It has close relationships with other topics like music transcription, remixing, and retrieval. A lot of early studies using deep learning models [@jansson2017singing;@takahashi2018mmdenselstm] perform separations in the frequency domain by estimating the ideal ratio masks (IRM) of the magnitude spectrogram and reusing the phase of the mixture. These approaches are effective but source phase variances are not well addressed. Later, time-domain models [@defossez2019demucs] start to demonstrate SoTA performance using direct waveform modeling, which does not involve transformations like short-time fourier transform (STFT). In this case, phase information can be implicitly estimated and models will not be restricted with the fixed time-frequency resolution. To enhance the MSS performance, a lot of models empoly complex architectures [@choi2019investigating;@liu2020voice] like self-attension. To alleviate the computational cost, @kadandale2020multi designs a multi-task model to replace source-dedicated models. Also, [@liu2020channel] proposes to use the channel-wise subband feature to cut down resource consumptions and improve separation performance by encouraging local weight sharing. Recently, [@kong2021decoupling] conducted experiments on the MSS system upper bound, which prove the limitation of IRMs and the importance of phase estimation. This paper introduces a new MSS model: CWS-PResUNet. We also describe ByteMSS, the system we submitted for the MDX Challenge [@mitsufuji2021music]. 
