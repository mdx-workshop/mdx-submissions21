...
# Introduction
Music source separation aims at separating a mixture track into several soundtracks, such as `Vocals`, `Bass`, `Drums`, and `Other` tracks. It has close relationships with other topics like music transcription, remixing, and retrieval. A lot of early studies using deep learning models [jansson2017singing;@hennequin2020spleeter] perform separations in the frequency domain by estimating the ideal ratio masks (IRM) of the magnitude spectrogram and re-utilizing the phase of the mixture. These approaches are effective but phase variances are not well addressed. Later, time-domain models [@defossez2019demucs] start to demonstrate SoTA performance by direct waveform modeling, which does not involve time-frequency transformations like short-time fourier transform (STFT). In this case, phase information can be implicitly estimated and models will not be restricted with the fixed time-frequency resolution. A lot of studies enhance the MSS performance by improving model architectures [@choi2019investigating;liu2020voice]. To alleviate the computational cost, @kadandale2020multi designs a multi-task model to replace source-dedicated models. Also, [@liu2020channel] proposes to use the channel-wise subband feature to cut down resource cost and enhance performance at the same time. Recently, [@kong2021decoupling] conducted experiments on the MSS system upper bound, which prove the limitation of IRMs and the importance of phase estimation. In this paper, we propose a new model, CWS-PResUNet, which conducts unbounded mask and phase variation estimation following @kong2021decoupling and utilize the CWS feature to reduce computational cost. We also discuss ByteMSS, the system we submitted for the MDX Challenge [@mitsufuji2021music]. 
